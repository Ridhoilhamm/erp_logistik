...
<div class="container mx-auto p-4">
    <div>
        <div class="flex items-center justify-between mb-4">
            <div>
                <h1 class="text-white text-2xl font-semibold">Stock Keluar</h1>
                <p class="text-gray-400">Silakan isi Formulir di bawah ini untuk mengeluarkan stock</p>
            </div>
            <div>
                <x-ts-button icon="fas.file-import" text="Import Excel" color="yellow" onclick="history.back()" wire:navigate light />
            </div>
        </div>
            ...
            <table class="min-w-full text-sm text-center text-gray-300">
                ...
                <tbody class="divide-y divide-gray-600">
                    @forelse($stockHistory as $history)
                        @if($history['type'] === 'out') <!-- Menampilkan hanya yang tipe 'in' -->
                            <tr class="hover:bg-gray-700">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium 
                                        {{ $history['type'] === 'out' ? 'bg-green-600 text-white' : 'bg-red-600 text-white' }}">
